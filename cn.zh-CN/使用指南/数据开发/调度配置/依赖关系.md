...
DataWorks V2.0提供[自动推荐](#)、[自动解析](#)和[自定义配置](#)三种依赖配置模式。依赖关系实际操作示例请参考[设置调度依赖最佳实践](../../../../../cn.zh-CN/最佳实践/设置调度依赖最佳实践.md#)。

![](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/16303/15475373207925_zh-CN.png)

无论如何配置依赖关系，调度的总逻辑不变：上游执行成功之后才会触发下游调度。因此，所有工作流节点都必须至少有一个父节点，调度依赖的核心就是设置这个父子依赖关系。下文将为您详细介绍调度依赖的原理及配置方式。

**说明：** 

-   如果您有**标准模式**和**简单模式**项目之间互相配置依赖关系的需求，请提工单申请。
-   您可以观看[DataWorks V2.0常见问题与难点分析](https://help.aliyun.com/video_detail/97879.html)学习依赖关系配置与排错。

## 规范化数据开发场景 {#section_nv5_p1y_p2b .section}

-   在进行调度依赖关系配置前，您需要了解以下**基本概念**：

    -   **DataWorks任务**：定义对数据执行的操作。详情请参见[基本概念](../../../../../cn.zh-CN/产品简介/基本概念.md#)。
    -   **输出名称**：系统将为每个节点默认分配一个以.out结尾的输出名，同时您也可增加自定义输出名，但需注意输出节点名称在租户内不允许重复。详情请参见[基本概念](../../../../../cn.zh-CN/产品简介/基本概念.md#)。
    ...
    每个形成上下游关系的任务均通过**输出名**进行关联，其中创建的最上游节点的上游节点可配置为本项目的**根节点**（节点名：projectname\_root）。
...
-   **规范化数据开发流程示例**
    ...
    ![](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/16303/154753732013593_zh-CN.jpg)
...
依赖的上游节点指当前节点依赖的父节点。此处需填写**上游节点的输出名称**（一个节点可同时存在多个输出名称，视情况填写您需要的输出即可），而非上游节点名。您可以手动搜索上游输出名进行添加，也可以通过SQL[血缘关系](cn.zh-CN/使用指南/数据开发/界面功能/血缘关系.md#)代码解析得到。

![](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/16303/15475373207926_zh-CN.png)
...
-   **通过输入父节点输出名搜索**
    ...
    ![](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/16303/154753732013932_zh-CN.png)

-   **通过输入父节点输出名的表名称进行搜索**

    通过该方法搜索必须保证父节点的某一个输出名，为本节点SQL语句中INSERT或CREATE之后的表名称，形如**projectname.表名**（此类输出名一般可通过自动解析获得）。

    ![](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/16303/154753732013933_zh-CN.png)

    执行**提交**后，该输出名即可通过搜索表名的方式被其他节点搜索到。

    ![](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/16303/154753732013934_zh-CN.png)
...
**说明：** 如果您是从DataWorksV1.0升级至DataWorks V2.0的用户，则本节点输出名称为**projectname.本节点名**。

如果出现的多个INSERT、FROM，则会自动解析出多个输出、输入名称。

![](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/16303/154753732013595_zh-CN.jpg)

如果您构建了存在依赖关系的多个任务且满足条件：下游任务的输入表均来自上游任务的输出表，则通过自动解析功能即可实现全工作流依赖关系的快速配置。

![](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/16303/154753732013935_zh-CN.png)
...
-   如果SQL代码中有临时表（例如在[属性配置](cn.zh-CN/使用指南/数据开发/配置管理/项目配置.md#)中指定t\_开头的表为临时表），则这个表不会被解析为调度依赖。

在自动解析的前提下，您可通过**手工设置**添加/删除、输入/输出的方式来决定避免/增加某些SQL语句中的字符被自动解析为输出名/输入名。

![](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/16303/154753732113937_zh-CN.png)
...
![](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/16303/154753732121179_zh-CN.png)
...
-   **手动添加依赖的上游节点示例**
    1.  新建三个节点，系统会默认为它们分别配置一个输出名称。
        ...
        ![](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/16303/154753732113938_zh-CN.png)

        ![](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/16303/154753732113939_zh-CN.png)

        ![](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/16303/154753732113940_zh-CN.png)

        ![](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/16303/154753732113941_zh-CN.png)

    2.  将最上游节点task\_1配置依赖本项目root根节点，单击**保存**。

        ![](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/16303/154753732113942_zh-CN.png)

    3.  配置task\_2依赖task\_1的输出名称，单击**保存**。

        ![](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/16303/154753732113943_zh-CN.png)

    4.  配置task\_3依赖task\_2的输出名称，单击**保存**。

        ![](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/16303/154753732113944_zh-CN.png)

    5.  配置完成后，单击**提交**，判断依赖关系是否正确。如果提交成功则说明依赖配置无误。

        ![](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/16303/154753732113945_zh-CN.png)
...
-   **通过拖拽形成依赖关系示例**
    ...
    2.  通过拖拽方式将三个task连接起来。
        ...
        ![](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/16303/154753732213947_zh-CN.png)

    3.  查看task\_2、task\_3的依赖配置，可看到已自动生成依赖的父节点输出名。

        ![](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/16303/154753732213948_zh-CN.png)

        ![](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/16303/154753732213949_zh-CN.png)
...
![](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/16303/154753732213952_zh-CN.png)
