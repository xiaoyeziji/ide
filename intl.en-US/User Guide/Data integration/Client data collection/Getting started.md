...
2.  Perform the following to configure the data synchronization task.
    ...
    After selecting the project name that maxcompute has created, enter a new custom data table name.

    **Note:** Not a data table name that has been created, only enter a new table name, the synchronization process automatically creates the data table.
...
|Access\_subtype|string|Network types such as: HSPA, evdo edge, GPRS, and so on.|
...
|localtimestamp|string|Terminal time \(UNIX time in numeric format, exactly to milliseconds, can be converted to date by power function\)|
|Utdid|string|Unique Identifier of the device generated by the server|
|user\_nick|string|Long logon member name, long logon means that you are remember the device's most recent logon member if you log on once, even if the device opens the app next time and is not logged on, its log also records the device's most recent sign-on membership|
|user\_id|string|Long logon membership ID|
|short\_user\_nick|string|Short sign-on member name. Short sign-on is the member currently in the logon state.|
|short\_user\_id|string|Short logon membership ID|
...
|1006|Register an event|Interface adonlyment userregister \(\) burial point|
