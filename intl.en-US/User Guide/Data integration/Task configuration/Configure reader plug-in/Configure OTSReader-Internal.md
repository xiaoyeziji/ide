...
-   Normal mode: Consistent with the normal mode of the hbase reader, export the latest version of each column in each row of data. For more information, see [Configure HBase reader](intl.en-US/User Guide/Data integration/Task configuration/Configure reader plug-in/Configure HBase reader.md#)the normal mode content that is supported by the hbase reader in.
