...
1.  STS模式授权
    ...
    -   当MaxCompute和OSS的项目所有者是同一个账号时，可以直接登录阿里云账号后一键授权。一键授权页面可通过数据开发/新建表时点击选取，如下图所示。
        ...
        ![](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/21811/155194787212719_zh-CN.png)

        ![](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/21811/155194787212720_zh-CN.png)
    ...
    -   自定义授权。 首先需要在RAM中授予MaxCompute访问OSS的权限。登录 RAM控制台（若MaxCompute和OSS不是同一个账号，此处需由OSS账号登录并授权），通过控制台中的**角色管理**创建**用户角色** ，角色名为AliyunODPSDefaultRole或AliyunODPSRoleForOtherUser。如下图所示：
        ...
        ![](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/21811/155194787212704_zh-CN.png)
...
2.  使用数据集成OSS 数据源
    ...
    ![](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/21811/155194787212714_zh-CN.png)
...
1.  DDL模式建表
    ...
    进入数据开发页面，参考[表管理](intl.zh-CN/使用指南/数据开发/表管理.md#)进行DDL模式建表，您需只需遵守正常的MaxCompute语法即可。如果您的STS服务已成功授权，可以不写odps.properties.rolearn属性。DDL建表语句举例如下所示，其中EXTERNAL参数说明此表为外部表。
    ...
    **SERDEPROPERTIES**序列化属性列表如下。MaxCompute目前只支持通过内置extractor读取OSS上gzip压缩的CSV/TSV数据，用户可以选择 文件是否是gzip压缩的。不同的文件格式对应不同的属性设置。
    ...
    ![](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/21811/155194787212823_zh-CN.png)
...
3.  图形化建表
    ...
    -   物理模型设计
        ...
        -   **选择存储地址**：即LOCATION参数。您可以在**物理模型设计**栏中设置LOCATION参数，如下图所示，该参数可在图形化界面**点击选择**，完成后可直接进行**一键授权**。
            ...
            ![](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/21811/155194787212744_zh-CN.png)
    ...
    -   表结构设计
        ...
        ![](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/21811/155194787212750_zh-CN.png)
...
![](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/21811/155194787312827_zh-CN.png)
