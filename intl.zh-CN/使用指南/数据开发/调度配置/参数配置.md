...
参数配置功能帮助您的任务在自动周期运行时能动态适配环境变化。您可以通过参数设置任务运行的时间、周期等属性。目前参数分为系统参数和自定义参数（推荐）两大类，本文为您分别列举两类参数的详细说明和操作示例。
...
|自定义参数\(**推荐使用**\)|在代码中引用$\{key1\},$\{key2\}， 然后在“参数”编辑框以如下方式设置 “key1=value1 key2=value2”，|非Shell节点类型|常量参数：param1=”abc” param2=1234； 变量参数：param1=$\[yyyymmdd\], 结果将基于bdp.system.cyctime的取值计算|
...
-   参数=两边不可以加空格。正确写法示例：`bizdate=$bizdate`
    ...
    ![](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/16301/15474547567838_zh-CN.png)

-   如果存在多个参数，每个参数用空格分开。

    ![](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/16301/15474547567839_zh-CN.png)
...
2.  完成上述步骤后，您的任务已经通过系统参数完成了分区的设置。接下来您可以设置[时间属性](intl.zh-CN/使用指南/数据开发/调度配置/时间属性.md#)与[调度依赖](intl.zh-CN/使用指南/数据开发/调度配置/依赖关系.md#)，本例中设置调度周期为小时。
    ...
    ![](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/16301/154745475637133_zh-CN.png)
...
3.  设置好周期与依赖后，提交任务，您可以在[运维中心](intl.zh-CN/使用指南/运维中心/任务列表/周期任务.md#)看到您的任务。从第二天开始，您的任务在运行时会产生周期实例，右键实例点击**查看运行日志**，即可看到系统参数被实际解析出来的时间。
    ...
    ![](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/16301/154745475637134_zh-CN.png)
...
1.  在代码中引用参数，代码如下。
    ...
    ![](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/16301/154745475737177_zh-CN.png)
...
2.  代码中引用的变量，需要**调度配置** \> **基本属性** \> **参数**中赋值。
    ...
    ![](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/16301/154745475737178_zh-CN.png)

3.  设置时间周期为每小时运行一次。

    ![](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/16301/154745475737179_zh-CN.png)
...
4.  设置好周期与依赖后，提交任务，您可以在[运维中心](intl.zh-CN/使用指南/运维中心/任务列表/周期任务.md#)看到您的任务。从第二天开始，您的任务在运行时会产生周期实例，右键实例点击**查看运行日志**，即可看到自定义参数被实际解析出来的时间：由于CYCTIME为20190114010000，因此解析出来的thishour时间为2019-01-14/01:00:00，而lasthour代表上一个小时，即2019-01-14/00:00:00。
    ...
    ![](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/16301/154745475737180_zh-CN.png)
...
1.  在代码中引用参数，代码如下。
    ...
    ![](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/16301/154745475737155_zh-CN.png)

2.  代码中引用的变量，需要**调度配置** \> **基本属性** \> **参数**中赋值。赋值规则：参数1 参数2 参数3…\(根据参数的位置，解析出来替换的变量，例如，$1解析出来的值是参数1\)。本例中分别给$1和$2赋值为abcd及$\[yyyy-mm-dd-2\]。

    ![](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/16301/154745475737156_zh-CN.png)

3.  设置时间周期为每天 1 点运行。

    ![](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/16301/154745475737162_zh-CN.png)
...
4.  设置好周期与依赖后，提交任务，您可以在[运维中心](intl.zh-CN/使用指南/运维中心/任务列表/周期任务.md#)看到您的任务。从第二天开始，您的任务在运行时会产生周期实例，右键实例点击**查看运行日志**，即可看到代码中 $1 被替换为常量 abcd，$2 被替换为 2019-01-12，也就是运行日期的前两天， $\{bdp.system.cyctime\} 被替换为 20190114010000。。
    ...
    ![](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/16301/154745475737175_zh-CN.png)


## 自定义参数变量 {#section_pcz_4ql_lgb .section}

自定义参数变量分为**固定值**和**调度内置参数**两种。
...
![](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/16301/15474547577844_zh-CN.png)
...
![](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/16301/15474547577847_zh-CN.png)

![](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/16301/15474547577848_zh-CN.png)

**$jobid**参数解释：任务所属的工作流ID。示例：jobid=$jobid。

**$nodeid**参数解释：节点ID。示例：nodeid=$nodeid。

**$taskid**参数解释：任务ID（节点实例ID）。示例：taskid=$taskid。
