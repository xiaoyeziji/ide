...
当出现数据集成问题时，您可以参见[添加数据源典型问题场景](https://help.aliyun.com/knowledge_detail/94430.html)进行排查。
...
3.  确认数据源测试连通性是否成功。
    ...
    参见[配置数据源](cn.zh-CN/使用指南/数据集成/数据源配置/支持的数据源.md#)文档，检查其数据源相关信息是否填错。通常填错的情况如下：
...
![](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/16281/15504708098678_zh-CN.png)

## 界面模式报错，复制排查码 {#section_n3w_ynb_r2b .section}

如果报错，请复制排查码，并提供给处理人员。

![](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/16281/15504708098679_zh-CN.jpg)
...
-   **ava.lang.Exception：DataX无法连接对应的数据库**
    ...
    **排查思路**：
    ...
    -   VPC环境的RDS-PostgreSQL不能运行在自定义资源组上，因为VPC环境的RDS有反向代理功能，这样与用户自定义资源组存在网络问题。因此，通常VPC环境的RDS直接运行在DataWorks默认的资源即可。如果默认资源不能满足您的需要，要运行在自己的资源上，可以将VPC环境的RDS作为VPC环境JDBC形式的数据源，购买一个同网段的ECS。**详情请参见 [VPC环境数据同步配置](https://help.aliyun.com/document_detail/56068.html)**。
...
-   **数据源信息填写错误**
    ...
    -   DRDS连接数据超时
        ...
        ![](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/16281/15504708098686_zh-CN.png)
    ...
    -   系统内部问题
        ...
        ![](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/16281/15504708098687_zh-CN.png)

        排查思路：

        通常是在开发环境将JSON格式该错了然后直接保存，保存在后通常就会报系统内部问题。界面显示为空白，碰到这样的问题，直接将您的项目名和节点名称提供给我们这边会后台处理的。
...
-   **脏数据**
    ...
    -   **JDBC的格式填错**
        ...
        ![](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/16281/15504708098688_zh-CN.png)
    ...
    -   **测试连通性失败**
        ...
        ![](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/16281/15504708098692_zh-CN.png)
